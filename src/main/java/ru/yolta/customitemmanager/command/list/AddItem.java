...
public final class AddItem implements SubCommand {
    ...
    private final MessageConfig.AddItemCmdSection cmdMessages;

    public AddItem(
            @NotNull MessageConfig.SharedCmdSection sharedMessages,
            @NotNull MessageConfig.AddItemCmdSection cmdMessages
    ) {
    ...
    public void onCommand(@NotNull CommandSender sender, @NotNull String @NotNull [] args) {
        ...
        if (args.length != 2) {
            Messenger.sendMessage(sender, sharedMessages.invalidArguments(), Map.of("USAGE", "/cim add <name>"));
            return;
        }
        ...
        Future.runAsyncTask(() -> CustomItemManager.getApi().registerCustomItem(itemName, item), success -> {
            if (success) {
                Messenger.sendMessage(sender, cmdMessages.itemRegistered(), Map.of("ITEM", itemName));
            } else {
                Messenger.sendMessage(sender, cmdMessages.itemAlreadyRegistered(), Map.of("ITEM", itemName));
            }
        });
    }
